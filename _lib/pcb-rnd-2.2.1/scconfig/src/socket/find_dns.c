...
int find_socket_getaddrinfo(const char *name, int logdepth, int fatal)
{
	...
	if (try_socket(logdepth, "libs/socket/getaddrinfo", inc_netdb, test_c, NULL, NULL, 1)) return 0;
	if (try_socket(logdepth, "libs/socket/getaddrinfo", inc_netdb_in, test_c, NULL, NULL, 1)) return 0;
	if (try_socket(logdepth, "libs/socket/getaddrinfo", inc_win32, test_c, NULL, NULL, 1)) return 0;
	return 1;
}
